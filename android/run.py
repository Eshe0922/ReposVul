...
import git
def clone_github_repo(repos_url, local_path):

    git.Repo.clone_from(repos_url, local_path)
    time.sleep(5)
    ...
    with open(write_filename, 'w', encoding='utf-8') as f:
        for CVE in CVES:
            ...
            for resource in CVE['resources']:
                if "android.googlesource" in resource:

                    merge_data = cve_info(CVE, patch_index)

                    repos_url = resource.partition('/+/')[0]
                    commit_id = resource.partition('/+/')[2].partition('%')[0]
                    repos_name = repos_url.partition('https://android.googlesource.com/')[2].replace('/','_')

                    local_path = os.path.join('./repos_now', repos_name)
                    
                    if not os.path.exists(local_path):
                        try:
                            clone_github_repo(repos_url, local_path)
                    ...
                    try:
                        ...
                        for parent_id in parent_result.split():
                            ...
                            parent['url_before'] = ''
                            parent['html_url_before'] = repos_url + '/+/' + parent_id
                    ...
                    try:
                        ...
                        for modified_file in modified_files:
                            detail['raw_url'] = repos_url + '/+/' + commit_id + '/' + modified_file
