...
def process_content(lock, line, new_list):
    ...
    try:
        ...
        run_code = 'timeout 300 cflow -T -d 2 --omit-symbol-names -r $(find {}/{} -type f -name "*.c")'.format(path_after, folder_name)
        ...
        if result1.returncode == 0:
            ...
            for t in tmp:
                ...
                run_code1 = 'timeout 300 cflow -T -m {} -d 2 --omit-symbol-names $(find {}/{} -type f -name "*.c")'.format(func_name, path_after, folder_name)
        ...
        traceback.print_exc()
    with lock:
        with open('/new_data/Challenge/newest/output/output_c.jsonl', 'a') as w1:
