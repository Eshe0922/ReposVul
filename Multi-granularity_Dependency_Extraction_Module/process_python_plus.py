...
import parse_getout_nearfunc_python_plus
import traceback
...
def process_content(lock, line, new_list):
    try:
        ...
        if len(json_file['caller']) == 0 and len(json_file['callee']) == 0:
            ...
            with lock:
                with open('/new_data/Challenge/newest/output/output_python_final.jsonl', 'a') as w1:
        ...
        if len(json_file['caller']) > 0:
            ...
            if 'line_numbers' in pre_json_line:
                for i in range(len(pre_json_line['line_numbers'])):
                    ...
                    final_output_json['caller_of_change'].update(parse_getout_nearfunc_python_plus.choose_caller(unzip_code_path, line_start, line_end, json_file['caller']))
        ...
        if len(json_file['callee']) > 0:
            ...
            for cc in changed_content:
                ...
                func_name = parse_getout_nearfunc_python_plus.get_func_name_from_code(json_line_1['function'])
...
with open('/new_data/Challenge/newest/output/output_python_final.jsonl', 'r', encoding = 'utf-8') as r2:
